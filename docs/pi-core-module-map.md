...
| CLI、print mode 与 RPC | `packages/coding-agent/src/{main,cli/*,modes/print-mode,modes/rpc/{rpc-mode,rpc-types,jsonl}}.ts` | `src/app.n`, `src/config/args.n`, `src/agent/event_json.n` | 44 个 e2e；event JSON 9/9 | Phase 6 进行中：piped stdin 挂起与 credential 输出隔离已关闭；help 文本逐项核对与参数矩阵收尾中 |
