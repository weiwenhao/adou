...
状态：MVP 核心路径已完成逐模块核对（Pi 基线 `0.82.1`, commit `cced6a21da273b26ee4a23a803680614bbe8dd1e`）；Phase 1–6 全部关闭，Phase 7 进行中（storage 已完成，server supervisor/协议/rpc_stream 验收中，见 `docs/phase7-storage-design.md` Phase 7.1），Pi extension 运行时已停用。
...
| Server IPC（Phase 7.1） | `packages/server/src/ipc/{protocol,server}.ts`, `supervisor.ts`, `handler.ts`, `rpc-process.ts`（进程管理按隔离方案排除） | `src/server/{protocol,supervisor,ipc_server}.n` + `src/app.n` run_serve/serve_command | `tests/ipc_protocol_test.n`；`tests/e2e/rpc-over-ipc.sh` 多实例/rpc_stream e2e | 进行中：spawn/list/status/stop/rpc/rpc_stream 响应形状与多实例生命周期已按上游协议落地，e2e 验收中；radius 继续排除；实例隔离采用进程内 runner（子进程 stdin 管道硬阻塞，证据见 phase7-storage-design） |
