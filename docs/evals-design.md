...
| `scripts/run-evals.mjs` | CLI 包装：校验 PI_PROVIDER/PI_MODEL 后跑 vitest | `Makefile` 的 `eval` target：经 `scripts/nature-build-safe.sh` guarded 串行构建 `tests/evals/smoke_evals.n` 并运行二进制 |

上游 harness 依赖真实模型 API（`PI_PROVIDER`/`PI_MODEL` + ModelRuntime），本实现按任务要求改为**本地脚本化 HTTP mock**（参照 `tests/anthropic_http_stream_test.n` / `deepseek_http_stream_test.n` 的 fixture server 模式）：每个 case 一个 `http.server()`，按请求序号返回预置的 Anthropic-messages SSE 体，全程离线、确定、零成本。真实 provider eval 未做（记录为剩余差异）。
