...
`grep` 优先使用系统 `rg`（支持 `--hidden`、glob、大小写和 literal/context/limit），不可用时回退到 POSIX grep；路径不存在返回 `Path not found`，`limit <= 0` 按 Pi 归一为 1；`find` 使用系统搜索器并把合法 glob 传给搜索器尽早过滤，目录含 `.gitignore` 时保留原生 ignore 优先级再由 Nature 做最终 glob 校验，非法 glob 传播搜索器错误，并排除 `.git`、`node_modules`；`ls` 使用排序的 `ls -A1p`，保留隐藏文件和目录 `/` 后缀，且在执行前区分不存在路径和非目录路径。路径参数统一相对 session cwd 解析，命令参数不通过 shell 拼接，避免模式和路径注入。
